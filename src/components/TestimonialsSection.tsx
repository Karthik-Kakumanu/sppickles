      ...
      <div className="mt-14 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {testimonials.map((testimonial, index) => (
          <motion.article
            ...
            className="rounded-2xl border border-[#eadfd5] bg-white/90 p-6 shadow-md backdrop-blur-sm"
          >
            <span className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#fff5f3] text-[#8b1e1e]">
              <Quote className="h-5 w-5" />
            </span>
            <p className="mt-5 text-sm leading-8 text-[#5f4a39]">"{testimonial.review}"</p>
            <p className="mt-6 font-heading text-2xl font-semibold text-[#241612]">
              {testimonial.name}
            </p>
          </motion.article>
        ))}
